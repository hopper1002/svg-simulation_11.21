...
def plot_gene_ot(adata, platform="visium", sorted_by='PI', input_data=None, size=2, 
              top_n=25, ncols_each_sheet=5, nrows_each_sheet=5, save_path=None, order="desc", which="top"):
    # 判断数据是否为稀疏矩阵，获取基因表达数据并转置
    if sp.issparse(adata.X):
        gene_data = adata.X.A.T
    else:
    ...
    # 根据排序方式处理基因列表
    if sorted_by in ["SEP", "SIG", "PI", "OT"]:
        pi = pd.DataFrame(adata.var[sorted_by].reset_index())
        ascending = True if order == "asc" else False
        sorted_list = pi.sort_values(by = sorted_by, ascending = ascending)
        sorted_list.columns = ["geneID",sorted_by]
        sorted_idx = sorted_list.index.values
        gene_data = gene_data[sorted_idx, :]
        # 选择top/bottom/middle
        if which == "top":
            gene_data = gene_data[:top_n]
            sorted_genename = sorted_list["geneID"].reset_index(drop=True)[:top_n]
            sorted_pi = sorted_list[sorted_by].reset_index(drop=True)[:top_n]
        elif which == "bottom":
            gene_data = gene_data[-top_n:]
            sorted_genename = sorted_list["geneID"].reset_index(drop=True)[-top_n:]
            sorted_pi = sorted_list[sorted_by].reset_index(drop=True)[-top_n:]
        elif which == "middle":
            start = len(gene_data)//2 - top_n//2
            gene_data = gene_data[start:start+top_n]
            sorted_genename = sorted_list["geneID"].reset_index(drop=True)[start:start+top_n]
            sorted_pi = sorted_list[sorted_by].reset_index(drop=True)[start:start+top_n]
        else:
            raise ValueError("which 参数只能为 'top', 'bottom', 'middle'")
    else:
        # ...原有 input_data 分支不变...
        adata.var_names = [i.upper() for i in list(adata.var_names)]
        assert input_data["genename"] is not None or input_data["qvalue"] is not None,"please set \" input_data.columns=['genename','qvalue']\" "
    ...
    for figs in trange(0, len(gene_data), ncols * nrows):
        ...
        for i in range(ncols):
            for j in range(nrows):
                ...
                if sorted_by == None:
                    genename = adata.var_names[n]
                elif sorted_by in ["SEP", "SIG", "PI", "OT"]:
                    genename = sorted_genename[n]
                    pi_value = sorted_pi[n]
                else:
                    genename = sorted_genename[n]
